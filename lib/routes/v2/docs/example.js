...
module.exports = app => {
	...
	fetch('https://www.ft.com/__origami/service/navigation/v2/menus')
    	.then(res => res.json())
		.then(json => {
			json['navbar-uk'].items = json['navbar-uk'].items.map(item => {
				return Object.assign({}, item, json['drawer-uk'].items[0].submenu.items.find(item2 => {
					return item2.label === item.label;
				}) || {});
			});
			console.log(JSON.stringify(json['navbar-uk'].items[1].submenu.items, null, 4));
